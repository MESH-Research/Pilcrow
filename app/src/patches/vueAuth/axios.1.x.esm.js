...
var axios_1_x = {
  ...
  interceptor: function(req, res) {
    ...
    if (req) {
      this.Vue.prototype.$axios.interceptors.request.use(
        function(request) {
    ...
    if (res) {
      this.Vue.prototype.$axios.interceptors.response.use(
        function(response) {
  ...
  http: function(data) {
    var http = this.Vue.prototype.$axios(data);
