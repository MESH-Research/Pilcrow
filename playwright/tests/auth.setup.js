...
setup('authenticate applicationAdmin', async ({ page, baseURL }) => {
  await saveContext(
    page,
    {
...
setup('authenticate publicationAdministrator', async ({ page, baseURL }) => {
  await saveContext(
    page,
    {
...
setup('authenticate regularUser', async ({ page, baseURL }) => {
  await saveContext(
    page,
    {
...
async function saveContext(page, user, path) {
  ...
  await expect(page.getByRole('button', { name: 'User Account Dropdown Toggle' })).toContainText(user.username)
