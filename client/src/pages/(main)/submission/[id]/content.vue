...
const { params } = useRoute("submission:content")
const { result } = useQuery(GET_SUBMISSION, params)

const { lang, iconSet } = useQuasar()
...
async function submitEnteredText() {
  try {
    await updateContent({ id: params.id, content: submissionContent.value })
