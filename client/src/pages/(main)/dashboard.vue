  ...
  submissions.value.filter(function (submission) {
    return (
      ["DRAFT", "INITIALLY_SUBMITTED", "AWAITING_REVIEW"].includes(
        submission.status
      ) === false && submission.my_role == SubmissionUserRoles.Reviewer
    )
  })
)
const coordinator_reviews = computed(() =>
  submissions.value.filter(function (submission) {
    return (
      ...
      submission.my_role == SubmissionUserRoles.ReviewCoordinator
    )
  })
)
const submitter_submissions = computed(() =>
  submissions.value.filter(function (submission) {
    return submission.my_role == SubmissionUserRoles.Submitter
  })
)
</script>
