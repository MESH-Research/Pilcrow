...
describe("LoginPage", () => {
  ...
  test("login action attempts mutation", async () => {
    ...
    expect((wrapper.vm as any).push).toHaveBeenCalledTimes(1)
    expect((wrapper.vm as any).push).toHaveBeenCalledWith("/dashboard")
  })
  ...
  test("login redirects correctly", async () => {
    ...
    expect(handler).toHaveBeenCalled()
    expect((wrapper.vm as any).push).toHaveBeenCalledTimes(1)
    expect((wrapper.vm as any).push).toHaveBeenCalledWith("/test-result")
  })
})
