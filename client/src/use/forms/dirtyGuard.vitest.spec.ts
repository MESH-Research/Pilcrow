...
import { type Mock, describe, test, vi, expect } from "vitest"
...
describe("useDirtyGuard composable", () => {
  ...
  test("allows a clean navigation to continue", async () => {
    ...
    let callback
    ;(onBeforeRouteLeave as Mock).mockImplementation((cb) => (callback = cb))
  ...
  test("Shows dialog appropriately and correctly responds to user feedback", async () => {
    ...
    ;(onBeforeRouteLeave as Mock).mockImplementation(
      (cb) => (dirtyGuardCallback = cb)
    )
  ...
  test("sets and removes window handlers", () => {
    ...
    let unmountCb
    ;(onUnmounted as unknown as Mock).mockImplementation(
      (cb) => (unmountCb = cb)
    )
