...
describe('test validation helpers', () => {
    test('getErrorMessageKey function', () => {
        ...
        expect(getErrorMessageKey({ $validator: '$externalResults', $message: 'externalMessage' })).toEqual('externalMessage');
        expect(getErrorMessageKey({ $validator: 'required', $message: 'messgage' })).toEqual('required');
    });
